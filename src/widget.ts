...
interface IViewerTask {
  user: string;
  title: string;
  status: 'pending' | 'approved' | 'completed' | 'paused';
  offline: boolean;
}

interface IState {
  streamerSummary: string;
  streamerTasks: IStreamerTask[];
  viewerTasks: {
    pending: IViewerTask[];
    approved: IViewerTask[];
  };
  progress: number;
  config: {
    taskLimit: number;
...
function parseStreamerTasks(tasksString: string): IStreamerTask[] {
...
function renderStreamerTasks(): void {
  const summaryElement = document.getElementById('streamer-summary') as HTMLElement;
  const taskListElement = document.getElementById('streamer-task-list') as HTMLElement;
...
function updateViewerTaskStatus(user: string, status: string): void {
...
function renderViewerTasks(): void {
  const taskListElement = document.getElementById('viewer-task-list') as HTMLElement;
  taskListElement.innerHTML = '';
...
function rejectViewerTask(viewerName: string): void {
  const pendingIndex = state.viewerTasks.pending.findIndex(t => t.user.toLowerCase() === viewerName.toLowerCase());
  if (pendingIndex === -1) {
...
function completeStreamerTaskByIndex(index: number): void {
...
function renderProgressBar(): void {
  ...
  const progressBar = document.getElementById('progress-bar') as HTMLElement;
  const progressBarText = document.getElementById('progress-bar-text') as HTMLElement;
...
function handleViewerPresence(username: string, action: 'join' | 'part'): void {
